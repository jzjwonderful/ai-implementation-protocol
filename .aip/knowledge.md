...
- 分类: deployment
- 状态: active
- 症状: 想让某个项目自带 aip 技能（不装到 `~/.claude/skills/`）；另外，0.3.0 之前装过 AIP 的老仓库升级后，新会话不再自动打印 OVERVIEW
...
- 适用范围: 项目级技能目录 `<repo>/.claude/skills/`（Codex 是 `<repo>/.codex/skills/`，Grok 没有这个约定）；升级到 0.3.0 的任何老仓库都要检查一遍 SessionStart 钩子和 pre-commit 的路径
- 最后复核: 2026-09-20
- 关联: K-002 / ADR-4
