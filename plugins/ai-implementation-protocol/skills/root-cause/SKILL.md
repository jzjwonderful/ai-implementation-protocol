...
8. **沉淀** —— 验证过的真因写入 `.aip/knowledge.md`（按顶部 `## 类目` 选分类，填全字段，含 `适用范围`）：先检索去重（像就并/加关联，不像才新增）；按捕获纪律定状态——过 review 自检清单可直接标 `active` 并在知会里给依据，证据不足才先写 `draft`；然后跑 `python <aip 技能目录>/scripts/aip_knowledge.py --repo-root .` 重建索引（aip 技能目录是本技能的同级目录 `../aip/`）；属决策的进 `decisions.md`，无关侧发现进 `inbox.md`。
